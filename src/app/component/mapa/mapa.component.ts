...
export class MapaComponent implements OnInit {
  ...
  llenarFormulario(place: any) {
    ...
    const getAdressComp = (type: any) => {
      for (const component of place.address_components) {
        if (component.types[0] === type) {
          return component[addresNameFormat[type]];
        }
      }
      return '';
    }
    ...
    Object.entries(componentForm).forEach(entry => {
      ...
      this.formMapas.controls[key].setValue(getAdressComp(value));
    });

    this.formMapas.controls['direccion'].setValue(getAdressComp('route') + ' ' + getAdressComp('street_number'));
  }
